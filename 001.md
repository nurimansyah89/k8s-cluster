...
Untuk memastikan bahwa timezone kita telah tepat, kita perlu melakukan setup timezone. Hal ini diperlukan di *production* saat kita akan melakukan cek log dab sebagainya.
```bash
sudo timedatectl
...
Opsional, namun dapat membantu kita untuk me-*maintain* server.
```bash
...
Sebagi contoh:
```bash
sudo hostnamectl set-hostname control-plane.k8s.io
```
...
cd
mkdir -p k8s && cd k8s
sudo kubeadm init --upload-certs --control-plane-endpoint=<hostname> --pod-network-cidr=10.0.0.0/16 | tee kubeadm-init.out
```
...
Lalu, cari `CALICO_IPV4POOL_CIDR`, hapus komentar dan ubah `value`-nya menjadi `10.0.0.0/16` (Sesuai dengan `--pod-network-cird` saat inisiasi control plane diatas)
![](./assets/003.png)
...
Pastikan **calico-\*** dan **coredns-\*** bekerja (*running*)
![](./assets/004.png)
...
# Clusters
192.168.0.1 control-plane.k8s.io
192.168.0.2 worker-1.k8s.io
...
```bash
kubeadm join control-plane.k8s.io:6443 --token 2zrdla.x5uuh63nu05ngceh \
...
### Terakhir, kembali pada node worker yang dihapus ###
```bash
sudo systemctl restart containerd
```
