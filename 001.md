...
### Port dan Firewall ###
Lalu, pastikan bahwa *firewall* sistem kamu tidak mem-blok port-port berikut:
- 6433/TCP
- 2379-2380/TCP
- 10250/TCP
- 10259/TCP
...
Untuk pengguna **ufw**, dapat melakukan ini:
```bash
ufw allow 6433/tcp
...
> Untuk metode yang lebih proper, silahkan allow IP address/IP range

\* Berdasarkan dari [dokumen resmi kubernetes](https://kubernetes.io/docs/reference/ports-and-protocols/).
...
Sebagi contoh:
```bash
sudo hostnamectl set-hostname control-plane.k8s.local
```
...
## Step 8: Install CRI (Container Runtime Inteface) ##
...
> CATATAN: Ganti `<hostname>` dengan hostname yang sudah kamu set diatas sebelumnya
...
sudo kubeadm init --upload-certs \
  ...
  --control-plane-endpoint=<hostname-diatas> \
  --pod-network-cidr=10.10.0.0/16 | tee kubeadm-init.out
```

> Jika kamu ingin melihat log inisasi kembali, jalankan perintah `cat kubeadm-init.out` pada direktori `$HOME/k8s`
...
> Maksud dari **Untaint** disini adalah, node **Control Plane** akan di treatment sebagai *worker* juga, sehingga aplikasi-mu juga akan terdeploy di pod didalam **Control Plane**. Biasanya, untuk kluster tipe **Single Node**, hal ini sangat lumrah dilakukan sehubungan dengan terbatasnya *resource* atau *cost*.
...
# Clusters
192.168.0.1 control-plane.k8s.local
192.168.0.2 worker-1.k8s.local
...
```bash
kubeadm join control-plane.k8s.local:6443 --token 2zrdla.x5uuh63nu05ngceh \
	--discovery-token-ca-cert-hash sha256:0a335e3f901235982eefdce25c05419acf890b158a5ef188021123200996ffc9
```

> INGAT: Saat jalankan perintah join `kubeadm join ...`, jangan lupa menggunakan akses `sudo`, contoh: `sudo kubeadm join control-plane.k8s.local:6443 --token 2zrdla.x5uuh63nu05ngceh --discovery-token-ca-cert-hash sha256:0a335e3f901235982eefdce25c05419acf890b158a5ef188021123200996ffc9`
...
```bash
watch -n2 'kubectl get all -n kube-system -o wide'
```
![](./assets/006.png)
